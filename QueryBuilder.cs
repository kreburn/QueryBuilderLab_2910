...
namespace QB_Lab
{
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            //so this basically is all you need to change, bc you need to create something from scratch and not just replace something
            var command = connection.CreateCommand();
            command.CommandText = $"INSERT INTO {typeof(T).Name} ({sb}) VALUES ({moreSB})";
            var reader = command.ExecuteNonQuery();

            ///********* BRANDON HELPED ME CORRECT MINE HERE, AS I DID NOT HAVE A SECOND STRING BUILDER T.T
        }
